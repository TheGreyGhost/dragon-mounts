      ...
      for (int shell = 0; shell < NUMBER_OF_SHELLS; ++shell) {
        ...
        for (int strandNumber = 0; strandNumber < numberOfStrands; ++strandNumber) {
          ...
          for (int ySegment = lowermostYsegment; ySegment <= uppermostYSegment; ++ySegment) {
            ...
            vertexBuffer.begin(GL11.GL_TRIANGLE_STRIP, DefaultVertexFormats.POSITION_COLOR);  // http://www.glprogramming.com/red/chapter02.html
            ...
            for (int vertex = 0; vertex < 5; ++vertex) {
              ...
              vertexBuffer.pos(xLower + segmentXlower, SEGMENT_HEIGHT * ySegment, zLower + segmentZlower)
                .color(0.8F * BRIGHTNESS_EACH_SHELL, 0.8F * BRIGHTNESS_EACH_SHELL,
                                          1.0F * BRIGHTNESS_EACH_SHELL, ALPHA_VALUE)
                      .endVertex();

              vertexBuffer.pos(xUpper + segmentX, SEGMENT_HEIGHT * (ySegment + 1), zUpper + segmentZ)
                          .color(0.8F * BRIGHTNESS_EACH_SHELL, 0.8F * BRIGHTNESS_EACH_SHELL,
                                 1.0F * BRIGHTNESS_EACH_SHELL, ALPHA_VALUE)
                          .endVertex();
