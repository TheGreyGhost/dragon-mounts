...
public class BreathFX extends Particle implements IEntityParticle {
  ...
  public Collection<Pair<EnumFacing, AxisAlignedBB>> moveAndResizeParticle(double dx, double dy, double dz, float newWidth, float newHeight) {
    worldObj.theProfiler.startSection("moveAndResizeEntity");
    AxisAlignedBB entityAABB = getEntityBoundingBox().offset(0, 0, 0);  // get a copy
    ...
    AxisAlignedBB collisionZone = entityAABB.addCoord(wDXneg, wDYneg, wDZneg)
                                            ...
                                            .addCoord(dx, dy, dz);
    List<AxisAlignedBB> collidingAABB = worldObj.getCollisionBoxes(null, collisionZone);
    ...
    setEntityBoundingBox(entityAABB);
  ...
  public boolean isInWater() {
    for (Pair<EnumFacing, AxisAlignedBB> collision : collisions) {
      if (worldObj.isMaterialInBB(collision.getSecond(), Material.WATER)) {
        return true;
      }
    }
    return false;
  ...
  public boolean isInLava() {
    for (Pair<EnumFacing, AxisAlignedBB> collision : collisions) {
      if (worldObj.isMaterialInBB(collision.getSecond(), Material.LAVA)) {
        return true;
      }
    }
    return false;
